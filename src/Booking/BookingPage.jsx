...
const BookingPage = () => {
  ...
  const allFilled = passengers.every(p => p.name.trim() && p.age.trim() && p.gender.trim()) && contact.email.trim() && contact.phone.trim();
    ...
    <div className="min-h-screen bg-gray-50 pt-28 pb-16 px-4 sm:px-6 lg:px-8">
      <div className="max-w-3xl mx-auto bg-white shadow-lg rounded-xl p-6 space-y-8">
        ...
        <div className="flex justify-between items-center">
          <h2 className="text-2xl font-bold text-Darkgreen">Confirm Your Booking</h2>
        ...
        <section className="space-y-2">
          ...
          <div className="flex justify-between text-gray-700"><span>Base Fare x {seats.length}</span><span>₹{bus.fareDetails?.baseFare * seats.length}</span></div>
          {bus.fareDetails?.tax > 0 && <div className="flex justify-between text-gray-700"><span>Tax</span><span>₹{bus.fareDetails?.tax}</span></div>}
          {bus.fareDetails?.serviceFee > 0 && <div className="flex justify-between text-gray-700"><span>Service Fee</span><span>₹{bus.fareDetails?.serviceFee}</span></div>}
        ...
        <section className="space-y-4">
          ...
          {seats.map((seatNo, idx) => (
            <div key={seatNo} className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-center bg-gray-50 p-3 rounded-lg">
              <span className="font-medium text-gray-700">Seat {seatNo}</span>
              <input
                type="text"
