...
import { 
  ...
  FaChevronDown, FaChevronUp, FaUser, FaPhone, FaCar, FaRoute,
  FaLocationArrow, FaRegCompass, FaShieldAlt, FaCouch,
  FaCoffee, FaTv, FaMusic, FaFirstAid, FaAward, FaCheck,
  FaChevronRight
} from 'react-icons/fa';
            ...
            <div className="p-6">
              <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-3">
                    <h3 className="text-xl font-bold text-gray-900">{bus.busName || bus.busId?.busName || 'Premium Bus Service'}</h3>
                    <span className="px-3 py-1 text-sm font-semibold text-Darkgreen bg-green-50 rounded-full">
                      {bus.busType || bus.busId?.type || 'AC Sleeper'}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-4 mb-3">
                    <div className="flex items-center text-gray-700">
                      ...
                      <span className="font-medium">{bus.availableSeats || bus.busId?.availableSeats || 0} Seats Available</span>
                    </div>
                    <div className="flex items-center text-gray-700">
                      <FaBus className="text-Darkgreen mr-2" />
                      <span className="text-sm">{bus.busNumber || bus.busId?.busNumber || 'Bus No. N/A'}</span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
                    ...
                    <div>
                      ...
                      <p className="text-lg font-semibold text-gray-900">{formatTime(bus.departureTime)}</p>
                      <p className="text-sm text-gray-600 flex items-center mt-1">
                        <FaMapMarkerAlt className="text-Darkgreen mr-1" />
                        {bus.source || bus.routeId?.source?.name || 'Source N/A'}
                      </p>
                    </div>
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-lg font-semibold text-gray-900">{formatTime(bus.arrivalTime)}</p>
                      <p className="text-sm text-gray-600 flex items-center justify-end mt-1">
                        {bus.destination || bus.routeId?.destination?.name || 'Destination N/A'}
                ...
                <div className="lg:text-right">
                  ...
                  <div className="text-sm text-gray-600 mb-3">
                    {bus.fareDetails?.tax > 0 && (
                      <span>Includes ₹{bus.fareDetails.tax} tax</span>
                    )}
                  </div>
                  <button
              ...
              <div className="mt-6 pt-4 border-t border-gray-100">
                <div className="flex flex-wrap gap-4">
                  {(bus.amenities || bus.busId?.amenities || []).map((amenity, index) => (
                    <div key={index} className="flex items-center text-gray-600 bg-gray-50 px-3 py-1.5 rounded-full">
                      <span className="text-Darkgreen mr-2">
                        {amenityIcons[typeof amenity === 'string' ? amenity : amenity.name] || <FaCheck />}
                      </span>
                        ...
                        <div className="bg-gray-50 p-4 rounded-lg">
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="flex items-start space-x-3">
                              ...
                              <div>
                                <p className="font-medium">{bus.routeId?.source?.name}</p>
                            ...
                            {bus.routeId?.viaStops?.map((stop, index) => (
                        ...
                        <div className="bg-gray-50 p-4 rounded-lg">
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="flex items-start space-x-3">
                              ...
                              <div>
                                <p className="font-medium">{bus.routeId?.destination?.name}</p>
                                <p className="text-sm text-gray-600">
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          {[
                            { icon: <FaCouch />, label: `${bus.availableSeats || 0} Seats Available` },
                            { icon: <FaSnowflake />, label: bus.busId?.type || 'Type N/A' },
                            { icon: <FaRoute />, label: 'Live Tracking Available' },
                            { icon: <FaShieldAlt />, label: 'Safety Measures' }
                          ].map((feature, index) => (
                            <div key={index} className="flex items-center space-x-2 text-gray-700">
                              <span className="text-Darkgreen">{feature.icon}</span>
                      ...
                      <div>
                        ...
                        <div className="space-y-3">
                          {[
                            { icon: <FaExchangeAlt />, text: 'Free cancellation up to 24 hours before departure' },
                            { icon: <FaClock />, text: 'Guaranteed on-time departure' },
                            { icon: <FaShieldAlt />, text: 'Safe and secure booking' }
                          ].map((policy, index) => (
                            <div key={index} className="flex items-start space-x-3">
                              <span className="text-Darkgreen mt-0.5">{policy.icon}</span>
                              <span className="text-sm text-gray-600">{policy.text}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                      ...
                      <div className="bg-white p-4 rounded-lg shadow-sm">
                        <div className="flex items-center justify-between">
                          <div className="space-y-1">
                            ...
                            <p className="font-medium">{bus.routeId?.source?.name || 'Source N/A'}</p>
                          </div>
                          <FaArrowRight className="text-Darkgreen" />
                          <div className="space-y-1 text-right">
                            <p className="text-sm text-gray-600">To</p>
                            <p className="font-medium">{bus.routeId?.destination?.name || 'Destination N/A'}</p>
                          </div>
                        </div>
                      </div>
                    </div>
