...
const SeatDrawer = ({ open, onClose, bus }) => {
  ...
  const mappedSeats = seats.map((seat) => {
    ...
    const baseCls = "w-8 h-8 flex items-center justify-center text-xs font-medium rounded m-1";
    const statusCls =
      seat.status === "booked"
    ...
    <Dialog open={open} onClose={onClose} className="fixed inset-0 z-40 flex">
      ...
      <div className="relative ml-auto w-full max-w-md bg-white rounded-l-2xl flex flex-col shadow-2xl mt-20 sm:mt-24">
        ...
        <div className="flex justify-between items-center p-4 border-b border-gray-200">
          <div>
            <h3 className="font-semibold text-lg text-gray-900">{bus.busName || bus.busId?.busName}</h3>
        ...
        <div className="p-4 flex-1 overflow-y-auto">
          ...
          <div className="mt-6 space-x-4 flex text-sm">
            <div className="flex items-center"><span className="w-4 h-4 bg-gray-200 rounded mr-1" /> Available</div>
            <div className="flex items-center"><span className="w-4 h-4 bg-red-300 rounded mr-1" /> Booked</div>
            <div className="flex items-center"><span className="w-4 h-4 bg-Darkgreen rounded mr-1" /> Selected</div>
          </div>
        </div>
        ...
        <div className="p-4 border-t border-gray-200">
          ...
          <button
            ...
            onClick={() => navigate('/booking', { state: { bus, seats: selected } })}
